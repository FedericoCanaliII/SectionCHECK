...
class GestioneMomentocurvatura:
    ...
    def _aggiorna_visualizzazione_sezione(self):
        """
    ...
    def populate_momentocurvatura_combobox(self):
        ...
        try:
            # 1. SALVIAMO LA SELEZIONE PRECEDENTE (ID della sezione)
            previous_data = None
            if combo.count() > 0:
                # Prendiamo il dato UserRole dell'item attualmente selezionato
                previous_data = combo.itemData(combo.currentIndex(), Qt.UserRole)
            ...
            # Lista dei nomi e degli indici da aggiungere
            items_to_add = [] # Lista di tuple (nome, index_reale)
