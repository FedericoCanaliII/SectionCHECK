...
nel pannello Pressoflessione (anteprima sezione).

Funzionalità:
  - Rendering di forme carpenteria (rettangolo, poligono, cerchio)
  - Fori disegnati con il colore dello sfondo (effetto buco)
  - Barre disegnate come cerchi pieni
  - Pan con tasto sinistro del mouse, zoom con rotella
  - Griglia e assi sovrapposti con QPainter
  - Auto-fit alla selezione di una sezione
"""
...
# ==============================================================================
# HELPER GEOMETRIA  (utilizzato anche da gestione per passare i dati)
# ==============================================================================

def triangola_poligono(pts: list) -> list:
    """
    Ear-clipping semplice per poligoni convessi/concavi senza buchi.
    Ritorna lista di triangoli [(a, b, c), ...].
    """
    n = len(pts)
    if n < 3:
        return []
    ...
    area_signed = sum(cross2(pts[0], pts[i], pts[i + 1]) for i in range(1, n - 1))
    indices     = list(range(n)) if area_signed > 0 else list(reversed(range(n)))
...
class PressoflessioneSezioneWidget(QOpenGLWidget):
    """
    Widget OpenGL per la visualizzazione della geometria della sezione
    nel pannello di pressoflessione.
    """
    ...
    def __init__(self, parent=None) -> None:
        ...
        self._shapes:  list = []  # {'type', 'pts'/'center'+'radius', 'fill', 'outline', 'lw', 'tris'}
        self._bars:    list = []  # {'cx', 'cy', 'r'}
        self._staffe:  list = []  # {'punti': [[x,y],...]}

        # Cache per reset_view
        ...
        # Bounds mondo corrente
        self.wx_min = self.wx_max = 0.0
        self.wy_min = self.wy_max = 0.0

        self.setMouseTracking(True)

        # Richiedi stencil buffer per la griglia nei fori
        fmt = QSurfaceFormat()
    ...
    def set_section_data(self, section_data: dict | None) -> None:
        """
        Aggiorna la geometria visualizzata.

        Parametri
        ---------
        section_data : dict con chiave 'elementi' nel formato progetto.
                       None per svuotare la vista.
        """
        ...
        for elem in carpenteria:
            ...
            if base == 'rettangolo':
                ...
                pts = [(min(x0, x1), min(y0, y1)),
                       ...
                       (max(x0, x1), max(y0, y1)),
                       (min(x0, x1), max(y0, y1))]
                tris = triangola_poligono(pts)
                self._shapes.append({'type': 'poly', 'pts': pts, 'tris': tris,
                ...
                self._shapes.append({'type': 'ellipse', 'cx': cx, 'cy': cy,
                                     'rx': rx, 'ry': ry,
                                     'fill': fill, 'outline': outline, 'lw': lw,
                                     'is_foro': is_foro})
    ...
    def reset_view(self) -> None:
        """Centra e ri-adatta la vista alla sezione corrente."""
    ...
    def _gl_shapes(self) -> None:
        for s in self._shapes:
            if s['type'] == 'poly':
                # Fill (sfondo per fori, colore per carpenteria)
                glColor4f(*s['fill'])
                for tri in s.get('tris', []):
                    ...
                    glEnd()
                # Griglia dentro il foro tramite stencil
                if s.get('is_foro'):
                ...
                if s.get('is_foro'):
                    self._gl_grid_in_hole(s)
                # Ridisegna outline sopra la griglia
                if s.get('is_foro'):
                    glColor4f(*s['outline'])
                    glLineWidth(s['lw'])
    ...
    def _gl_grid_in_hole(self, shape: dict) -> None:
        """Ridisegna la griglia solo dentro il foro tramite stencil buffer."""
        glEnable(GL_STENCIL_TEST)
        glStencilMask(0xFF)
        glClear(GL_STENCIL_BUFFER_BIT)

        # 1. Scrivi la forma del foro nello stencil (senza scrivere colore)
